...
typedef NS_ENUM(NSInteger, WHPrinterStyle) {
    WHPrinterStyleDefault,
    WHPrinterStyleCustom
};

/** 文字对齐方式 */
typedef NS_ENUM(NSInteger, WHTextAlignment) {
    WHTextAlignmentLeft = 0x00,
    WHTextAlignmentCenter = 0x01,
    WHTextAlignmentRight = 0x02
};

/** 字号 */
typedef NS_ENUM(NSInteger, WHFontSize) {
    WHFontSizeTitleSmalle = 0x00,
    WHFontSizeTitleHSmalle = 0x01,
    WHFontSizeTitleMiddle = 0x11,
    WHFontSizeTitleHMiddle = 0x12,
    WHFontSizeTitleBig = 0x22
};

/** 加粗模式 */
typedef NS_ENUM(NSInteger, WHFontStyle) {
    WHFontStyleBoldCancel = 0x00,  //取消加粗
    WHFontStyleBold = 0x01         //加粗
};

/** 切纸模式 */
typedef NS_ENUM(NSInteger, WHCutPaperModel) {
    WHCutPaperModelFull = 0x00,
    WHCutPaperModelHalf = 0x01,
    WHCutPaperModelFeedPaperHalf = 0x66
};

@interface WHEscCommand : NSObject
 ...
 */
- (void)appendQRCodeWithInfo:(NSString *)info centerImage:(UIImage * _Nullable)centerImage alignment:(WHTextAlignment)alignment maxWidth:(CGFloat )maxWidth;
...
/**
 ...
 @param data 自定义的data
 */
- (void)appendCustomData:(NSData *_Nullable)data;
